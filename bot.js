...
const positions = {
    ...
    cobbleStoneDirection: { pitch: 0, yaw: 4.75 },
};
...
rl.on("line", (input) => {
    if (input.startsWith("-")) {
        ...
        switch (input) {
            ...
            case "pitchyaw": {
                log("pitch: " + bot.entity.pitch + " | " + bot.entity.yaw);
                break;
            }
            case "stop": {
                shouldStop = true;
...
function goToPosition(positionName, position) {
    log(chalk.yellow("going to " + positionName));
...
async function depositCobblestoneToMarket() {
    ...
    for (const chestToOpen of chests) {
        ...
        if (hasPutCobbleStone) {
            bot.chat("Cobble satışımız devam ediyor, cobblestoneler yenilendi /is warp GmodderTR 1 stack 10 tl");
        }

        await sleep(500);
    }

    log(chalk.greenBright("end putting cobblestones"));
}
...
async function dig() {
    ...
    if (bot.heldItem && bot.heldItem.name.includes("pickaxe")) {
        await bot.look(positions.cobbleStoneDirection.yaw, positions.cobbleStoneDirection.pitch);
    ...
    bot.on(
        ...
        function (username, message) {
            ...
            if (message.toLowerCase().includes("başarıyla giriş yaptın")) {
                setTimeout(() => {
                    if (bot.entity.position.distanceTo(positions.cobbleStoneMining) > 100) {
                        bot.chat("/is go");
                    } else {
                        if (bot.inventory.findInventoryItem(14)) {
                            goToCobblestoneMarket();
                        } else {
                            goToChest();
                        }
